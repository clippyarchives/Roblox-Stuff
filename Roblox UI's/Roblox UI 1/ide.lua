...
local grid = make(setc, "Frame", "grid"); grid.Position = UDim2.new(0,0,0,headh+pad); grid.Size = UDim2.new(1,0,1,-(headh+pad+48)); grid.BackgroundTransparency = 1;
local gl = Instance.new("UIGridLayout"); gl.Parent = grid; gl.FillDirection = Enum.FillDirection.Horizontal; gl.HorizontalAlignment = Enum.HorizontalAlignment.Left; gl.VerticalAlignment = Enum.VerticalAlignment.Start; gl.CellPadding = UDim2.new(0,10,0,10); gl.FillDirectionMaxCells = 3; gl.SortOrder = Enum.SortOrder.Name; gl.CellSize = UDim2.new(1/3,-14,0,64);
for i=1,#list do
  local k = list[i]; local bx = make(grid, "Frame", k.."_bx"); bx.Size = UDim2.new(0,0,0,64); bx.BackgroundColor3 = Color3.fromRGB(26,26,30); bx.BorderSizePixel = 0; local cr = Instance.new("UICorner"); cr.CornerRadius = UDim.new(0,4); cr.Parent = bx; local ss = Instance.new("UIStroke"); ss.Color = Color3.fromRGB(48,48,54); ss.Thickness = 1; ss.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; ss.Parent = bx;
  local l = make(bx, "TextLabel", k.."_l"); l.Size = UDim2.new(0,60,0,20); l.Position = UDim2.new(0,10,0,8); l.BackgroundTransparency = 1; l.TextXAlignment = Enum.TextXAlignment.Left; l.Font = Enum.Font.Code; l.TextSize = 16; l.TextColor3 = Color3.fromRGB(180,180,188); l.Text = k;
  local tb = make(bx, "TextBox", k.."_tb"); tb.Size = UDim2.new(1,-90,0,24); tb.Position = UDim2.new(0,70,0,8); tb.BackgroundColor3 = Color3.fromRGB(18,18,22); tb.BorderSizePixel = 0; tb.Font = Enum.Font.Code; tb.TextSize = 16; tb.TextColor3 = Color3.fromRGB(200,200,205); tb.ClearTextOnFocus = false; tb.Text = defcol[k] or "#FFFFFF"; local c2 = Instance.new("UICorner"); c2.CornerRadius = UDim.new(0,4); c2.Parent = tb; flds[k]=tb;
end
