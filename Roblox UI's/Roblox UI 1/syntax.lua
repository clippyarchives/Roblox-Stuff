...
syn.kw = {
  ...
  ["nil"]=true,["not"]=true,["or"]=true,["repeat"]=true,["return"]=true,["then"]=true,["true"]=true,["until"]=true,["while"]=true
};
...
function syn.hl(s)
  ...
  s = s:gsub("(%b\"\")", function(m)
    return hold(segs, "<font color=\""..syn.col.str.."\">"..m.."</font>");
  end);
  s = s:gsub("(%b'')", function(m)
    return hold(segs, "<font color=\""..syn.col.str.."\">"..m.."</font>");
  end);
  s = s:gsub("(%-%-[^\n]*)", function(m)
    return hold(segs, "<font color=\""..syn.col.com.."\">"..m.."</font>");
  end);
  ...
  s = s:gsub(sent.."(%d+)"..sent, function(i)
    return segs[tonumber(i)] or "";
  end);
  return s;
end
return syn;